...
class Student(persistent.Persistent):
    ...
    def printTranscript(self):
        ...
        transcript_html = "<html><head><title>Transcript</title></head><body>"

        # Add student information to the HTML content
        transcript_html += f"<h1>Transcript</h1>"
        transcript_html += f"<p>ID: {self.id}</p>"
        transcript_html += f"<p>Name: {self.name}</p>"

        # Start the course list table
        transcript_html += "<table>"
        transcript_html += "<thead><tr><th>Course Code</th><th>Course Name</th><th>Credits</th><th>Score</th></tr></thead><tbody>"
        ...
        for i in range(size):
            ...
            # Add course details to the HTML content
            transcript_html += f"<tr><td>{self.enrolls[i].course.id}</td>"
            transcript_html += f"<td>{self.enrolls[i].course.name}</td>"
            transcript_html += f"<td>{self.enrolls[i].course.credit}</td>"
            transcript_html += f"<td>{self.enrolls[i].getGrade()}</td></tr>"

            sum += mul

        # Close the course list table
        ...
        # Add GPA to the HTML content
        transcript_html += f"<p>Total GPA is: {gpa:.2f}</p>"
...
root.students[1100] = Student([], 1100, "Miss. Urawee Thani", "hell")
